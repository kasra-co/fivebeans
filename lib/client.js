...
var DEFAULT_RECONNECT = 1;
var DEFAULT_RECONNECT_INTERVAL = 5000;
...
var FiveBeansClient = function(host, port, options)
{
	...
	this.reconnect_interval = options.reconnect_interval || DEFAULT_RECONNECT_INTERVAL;
...
FiveBeansClient.prototype.connect = function()
{
	...
	self.stream.on('connect', function(cc)
	{

		self.connected = true;

		if(self.reconnect_timer) {
			self.reconnect_timer = clearTimeout(self.reconnect_timer);
	...
	self.stream.on('close', function(err)
	{
		...
		if(self.reconnect && !self.connected) {
			self.emit('reconnecting',self.reconnect_interval);
			if(!self.reconnect_timer) {
				self.reconnect_timer = setInterval(function(){
					self.end();
					self.connect();
